...
export class Game {
  ...
  updateFighter(fighter, input, isPlayer) {
    ...
    if (fighter.flashStunFrames > 0) {
      ...
      if (!fighter.flashStunned && fighter.state === "hitstun") { fighter.stunFrames = 0; fighter.state = fighter.grounded ? "idle" : "jumping"; }
    }
  ...
  activateSkill(fighter, config, resourceMode = "native") {
    ...
    if (type === "copy") {
      ...
      fighter.flashHitUsed = false; fighter.flashReloading = false; this.spawnSkillEntity({ owner, type: "flash", x: fighter.x + fighter.facing * 35, y: fighter.y + 76, duration: 3, damage: 0, w: 95, h: 85, guardable: true, justGuardable: true, oneHit: true, effectId: config.effectId });
    }
  }
  ...
  spawnVfx(effectId, fighterOrPoint = null, overrides = {}) {
    ...
    const record = {
      ...
      scale: Number(overrides.scale || descriptor.scale || 1), frames: Number(overrides.frames || descriptor.durationFrames || 1), age: 0, owner: fighterOrPoint?.id || null, hitbox: null,
    };
  ...
  spawnSkillEntity(entity = {}) {
    ...
    if (record.effectId) this.spawnVfx(record.effectId, { x: record.x, y: record.y }, { scale: record.scale || 1 });
    ...
    for (const entity of this.skillEntities) {
      ...
      if (!entity.hit && targetHit && !(entity.hitTargets?.has(defender.id))) {
        ...
        if (entity.type === "flash") {
          ...
          defender.flashComboHit = true; defender.flashStunned = true; defender.flashStunFrames = Math.min(180, Number(entity.maxHitstopFrames || 180)); defender.state = "hitstun"; defender.action = "hit_light"; defender.stunFrames = defender.flashStunFrames; this.onHit(attacker, defender, move, false, false, 0); this.spawnVfx("skill-flash", defender); continue;
        }
